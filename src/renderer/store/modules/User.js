...
let state = {
  ...
  userInfo: {},
  userDetail: '',
  userCollectists: [], // 用户收藏的歌单
  likedsongIds: [] // 喜欢的歌曲id列表 v
}
let getters = {
  ...
  userCollectists: state => state.userCollectists,
  subscribedList: state => {
    return state.userCollectists.filter(item => {
  ...
  likedPlaylistIds: state => state.userCollectists.map(item => item.id),
...
let actions = {
  ...
  async subscribePlatlist ({ commit, state }, { t, playlist }) {
    ...
    if (res.code === 200) {
      let userCollectists = state.userCollectists.slice()
      if (t === 1) {
        Message.success('收藏成功!')
        playlist.subscribed = true
        userCollectists.unshift(playlist)
      } else if (t === 2) {
        Message.success('取消收藏成功!')
        let index = userCollectists.findIndex(item => item.id === pid)
        userCollectists.splice(index, 1)
      }
      commit('SET_USER_PLAYLISTS', userCollectists)
    }
  },
  // 取消收藏歌单
  ...
  async deletePlaylist ({ commit, state }, id) {
    ...
    if (res.code === 200) {
      let userCollectists = state.userCollectists.slice()
      let index = userCollectists.findIndex(item => item.id === id)
      userCollectists.splice(index, 1)
      commit('SET_USER_PLAYLISTS', userCollectists)
      Message.success('删除成功!')
    }
  ...
  async createPlaylist ({ commit, state }, formData) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (res.code === 200) {
          let userCollectists = state.userCollectists.slice()
          userCollectists.push(res.playlist)
          commit('SET_USER_PLAYLISTS', userCollectists)
